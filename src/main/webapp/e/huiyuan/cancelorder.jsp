            ...
            <div>
                ...
                <table width="100%" border="0" cellspacing="0" cellpadding="0" class="ui-record-table">
                    ...
                    <tr v-if="order.state==1">
                        <td colspan="4">
                            <button @click="cancelHandler(order.id)" class="btn btn-danger">
...
<script type="text/javascript">
    ...
    let vm = new Vue({
        ...
        methods: {
            ...
            async cancelHandler(oid) {

                if (!confirm("确定要关闭订单?")) {
                    return;
                }
                let {data: res} = await this.$http.post("admin/shorder/cancel", {id: this.id});
                if (res.stateCode < 0) {
                    this.$message.error(res.des);
                    return;
                }
                this.$message.success("取消成功");
                window.location.href = this.hostHead + "/e/huiyuan/cancelres.jsp"
